...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    exec: {
      ...
      smoke_play: {
        maxBuffer: Infinity,
        cmd: 'find -L test/swf -name "*.swf" | parallel --gnu -X -N50 --timeout 200% utils/jsshell/js build/ts/shell.js -x -md 1000 -v -tp 60 -v {}'
      },
      ...
      test_swf_avm2_all: {
        maxBuffer: Infinity,
        cmd: 'mongo ats --eval \'db.swfs.find({"parse_result.uses_avm1": false}).forEach(function (x) { print("test/ats/swfs/" + x.file); })\' | parallel -k --gnu -X -N10 --timeout 200% utils/jsshell/js build/ts/shell.js -x -fc 10 {} | tee test/ats/test_swf_avm2_all.run;'
      },
      ...
      test_avm2_acceptance_parse: {
        maxBuffer: Infinity,
        cmd: 'find -L test/avm2/acceptance -name "*.abc" | parallel -k --gnu -X -N50 --timeout 200% utils/jsshell/js build/ts/shell.js -d -v {}'
      },
      // Runs SWFs and tests against the current baseline. If you get more tests to pass, update the baseline.
      test_swf_acceptance: {
        maxBuffer: Infinity,
        cmd: 'find -L test/swf -name "*.swf" | parallel -k --gnu -X -N1 --timeout 200% utils/jsshell/js build/ts/shell.js -x -fc 10 {} | sort > test/swf/acceptance.run && ' +
      ...
      test_arch_acceptance: {
        maxBuffer: Infinity,
        cmd: 'find -L test/arch/swfs -name "*.swf" | parallel --gnu -X -N1 --timeout 200% utils/jsshell/js build/ts/shell.js -x -fc 10 {} | tee test/arch/acceptance.run;' +

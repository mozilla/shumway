...
module Shumway.Player {
  ...
  export function embed(swfURL: string) {
    ...
    loaderInfo.addEventListener(flash.events.ProgressEvent.PROGRESS, function fn() {
      var root = loader.content;
      if (!root) {
        return;
      }
      loaderInfo.removeEventListener(flash.events.ProgressEvent.PROGRESS, fn);
      stage.frameRate = loaderInfo.frameRate;
      stage.stageWidth = loaderInfo.width;
      stage.stageHeight = loaderInfo.height;
      stage.addChild(root);
      stage.enterEventLoop();
    });

    loader.load(new flash.net.URLRequest(swfURL));
  }
}

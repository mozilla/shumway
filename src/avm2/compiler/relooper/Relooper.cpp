...
void Block::SetCode(const char *CodeInit) {
  if (Code) free((void*)Code);
...
RELOOPERDLL_API void rl_set_block_code(void *block, const char *text) {
#if DEBUG
  printf("  rl_relooper_block_set_code(rl, block_map[%d]);\n", ((Block*)block)->Id);
#endif
  ((Block*)block)->SetCode(text);
}

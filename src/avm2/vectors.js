...
var TypedArrayVector = (function () {
  ...
  vector.callable = function (object) {
    ...
    var length = object.getMultinameProperty(undefined, "length");
    if (length !== undefined) {
      var v = new vector(length, false);
      for (var i = 0; i < length; i++) {
        v.indexSet(i, object.indexGet(i));
      }
      return v;
    }
    unexpected();
  ...
  vector.prototype.every = function (callback, thisObject) {
    for (var i = 0; i < this._length; i++) {
      if (!callback.call(thisObject, this.indexGet(i), i, this)) {
        return false;
      }
    }
    return true;
  ...
  vector.prototype.forEach = function (callback, thisObject) {
    for (var i = 0; i < this._length; i++) {
      callback.call(thisObject, this.indexGet(i), i, this);
    }
  };
  ...
  vector.prototype.indexGet = function (i) {
    return this.buffer[this.offset + i];
  };

  vector.prototype.indexSet = function (i, v) {
    if (i === this._length) {
  ...
  vector.prototype._spliceHelper = function (index, insertCount, deleteCount, args, offset) {
    ...
    for (var i = 0; i < insertCount; i++) {
      this.buffer[this.offset + index + i] = args.indexGet(offset + i);
    }
  };
  // <<-- COPY UNTIL HERE
  return vector;
...
var GenericVector = (function () {
  ...
  vector.prototype._spliceHelper = function (index, insertCount, deleteCount, args, offset) {
    ...
    var items = [];
    for (var i = 0; i < insertCount; i++) {
      items.push(this._coerce(args.indexGet(offset + i)));
    }
